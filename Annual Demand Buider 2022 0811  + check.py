...
# 2022 0810： found a bug. In previous version, 2025 CAGR is 2023-2025. 
# Then in annual workflow, 2023 is culculated by 2022*(1+2023-2025 cagr), which is not correct.
# In this version, added 2022/2023 growth rate.

##  Found that there is no need to calculate cagr. Directly using growth rate is better and more accurate. 
# 2022 0811: replace CAGR with annual growth rate. n/n-1 
# more convenient and accurate in annual workflow.
...
table1 = pd.read_excel(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Input Data 2022 0811-run.xlsx', sheet_name = 'Target2')
...
yrs = pd.read_excel(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Input Final Scenario 2022 0811.xlsx', sheet_name = 'Yr_Index')

# COM
data = table.drop(['lelc', 'lind', 'lres', 'ltrn', 'lpet', 'lgdp_man', 'lche', 'lfoo', 'lmac', 'ltex', 'loth', 'lgasprice', 'loilprice'], axis = 1)

r_table = pd.read_excel(r'C:\\Users\\jiaxi\\OneDrive\\Desktop\\RBAC 2022\\Demand\\Demand Annual Builder\\2022 0811\\Input Scenarios 2022 0811.xlsx', sheet_name = 'COM')
    ...
    if i != 'ALL':
        if i in ref_country:
                ...
                for i in aclist:
                    ...
                    p_export.columns = ["Year", "lcom"]
                    
                    history2022 = country_data[country_data["Year"] == 2022] # the latest history year
                    history2022 =history2022[["Year", "lcom"]]
                    p_export = pd.concat([history2022, p_export])
                    ...
                    cagr['GR'] = (cagr['value'] / cagr['value'].shift(+1) ) - 1
                    
                    cagr2022 = cagr[cagr["Year"] == 2022]
                    cagr2050 = cagr[cagr["Year"] == 2050]
                    cagr_check = pd.concat([cagr2022, cagr2050])
                    cagr_check = cagr_check.drop(['GR'], axis = 1)
                    cagr_check['GR'] = (cagr_check['value'] / cagr_check['value'].shift(+1) ) ** (1/28) - 1 # 2050 - base year
                    cagr_check = cagr_check.drop(cagr_check[cagr_check.Year == 2022 ].index) # the base year that you would like to calculate cagr
...
df3["Sector"] = 'com'
df3.columns = ['CAGRYear', 'lvalue', 'Country ID', 'RegType', 'value', 'GR', 'Sector']             
df3 = pd.merge(df3,comcountry,on='Country ID',how='inner') 

df4["Sector"] = 'com'
df4.columns = ['CAGRYear', 'lvalue', 'Country ID', 'RegType', 'value', 'GR', 'Sector']             
...
r_table = pd.read_excel(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Input Scenarios 2022 0811.xlsx', sheet_name = 'IND')
    ...
    if i != 'ALL':
        if i in ref_country:
                ...
                for i in aclist:
                    ...
                    yr_p = yrs[yrs['Year']>=2023]   # first forecast year          
                    p_export = pd.concat([yr_p, y_hat], axis=1, sort=False)
                    p_export.columns = ["Year", "lind"]
                    
                    history2022 = country_data[country_data["Year"] == 2022] # the latest history year
                    history2022 =history2022[["Year", "lind"]]
                    p_export = pd.concat([history2022, p_export])
...
r_table = pd.read_excel(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Input Scenarios 2022 0811.xlsx', sheet_name = 'IND2')
...
#df2["Sector"] = 'ind'
#df2.columns = ['Year', 'lvalue', 'Country ID', 'RegType', 'value', 'Sector']   
#df2 = pd.merge(df2,ind2country,on='Country ID',how='inner')      
...
r_table = pd.read_excel(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Input Scenarios 2022 0811.xlsx', sheet_name = 'IND3')
...
r_table = pd.read_excel(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Input Scenarios 2022 0811.xlsx', sheet_name = 'RES')
    ...
    if i != 'ALL':
        if i in ref_country:
                ...
                for i in aclist:
                    ...
                    yr_p = yrs[yrs['Year']>=2023] # first forecast year             
                    p_export = pd.concat([yr_p, y_hat], axis=1, sort=False)
                    p_export.columns = ["Year", "lres"]
                    
                    history2022 = country_data[country_data["Year"] == 2022] # the latest history year
                    history2022 =history2022[["Year", "lres"]]
                    p_export = pd.concat([history2022, p_export])
                    p_export['Country ID'] = rid
                    p_export["RegType"] = a
...
r_table = pd.read_excel(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Input Scenarios 2022 0811.xlsx', sheet_name = 'TRN')
    ...
    if i != 'ALL':
        if i in ref_country:
                ...
                for i in aclist:
                    ...
                    yr_p = yrs[yrs['Year']>=2023]  # first forecast year           
                    p_export = pd.concat([yr_p, y_hat], axis=1, sort=False)
                    p_export.columns = ["Year", "ltrn"]
                    
                    history2022 = country_data[country_data["Year"] == 2022] # the latest history year
                    history2022 =history2022[["Year", "ltrn"]]
                    p_export = pd.concat([history2022, p_export])
...
order = ['Country ID','Country', 'Sector', 'RegType', 'RegID', 'CAGRYear', 'lvalue', 'value', 'GR']
totalcagr = tem.reindex(columns = order)
...
tem = pd.concat(tem,ignore_index=True)
order = ['Country ID','Country', 'Sector', 'RegType', 'RegID', 'CAGRYear', 'lvalue', 'value', 'GR']
totalcagr_check = tem.reindex(columns = order)

totalresult.to_csv(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Output - Total Results 2022 0811.csv',index=None)
#totalforecast.to_csv(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Output - Total Forecast 2022 0811.csv',index=None)
totalregID.to_csv(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Output - Total RegID 2022 0811.csv',index=None)
totalcagr.to_csv(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Output - Total CAGR 2022 0811.csv',index=None)
totalcagr_check.to_csv(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Output - Total CAGR Check 2022 0811.csv',index=None)
...
########## final selection
select = pd.read_excel(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Input Final Scenario 2022 0811.xlsx', sheet_name = 'Map')
...
selectcagr.sort_values(["Country", "Sector", "CAGRYear"] , inplace=True, ascending=True) 
...
selectcagr_check.sort_values(["Country", "Sector", "CAGRYear"] , inplace=True, ascending=True) 
...
selectresult.to_csv(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Output - Final Results 2022 0811.csv',index=None)
#selectforecast.to_csv(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Output - Final Forecast 2022 0811.csv',index=None)
selectcagr.to_csv(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Output - Final CAGR 2022 0811.csv',index=None)
selectcagr_check.to_csv(r'C:\Users\jiaxi\OneDrive\Desktop\RBAC 2022\Demand\Demand Annual Builder\2022 0811\Output - Final CAGR Check 2022 0811.csv',index=None)
